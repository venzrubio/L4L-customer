...
import { IonRouterOutlet, MenuController, NavController, Platform , ActionSheetController } from '@ionic/angular';
  ...
  initializeApp() {
    this.platform.ready().then(() => {
        ...
        this.api.get('users/getDefaultSettings').subscribe((data: any) => {
          ...
          if (data && data.status === 200 && data.data) {
            ...
            if (general && general.length > 0) {
              ...
              this.cart.originalOrderTax = parseFloat(info.tax);
              this.cart.freeShipping = parseFloat(info.free);
              this.cart.minimumDeliveryDistance = parseFloat(info.minimumDeliveryDistance);
              this.cart.minimumDeliveryRate = parseFloat(info.minimumDeliveryRate);
              this.cart.minimumServiceCharge = parseFloat(info.minimumServiceCharge);
              this.cart.pickupServiceCharge = parseFloat(info.pickupServiceCharge);
            }
          }
        }, error => {
      ...
      if (this.platform.is('cordova')) {
        setTimeout(async () => {
          await this.oneSignal.startInit(environment.onesignal_customer.appId, environment.onesignal_customer.googleProjectNumber);
...
//ios
      ...
      this.platform.backButton.subscribe(async () => {
        ...
        if (this.router.url === '/tabs/categories' ||
          this.router.url === '/tabs/orders' || this.router.url === '/tabs/account'
          || this.router.url === '/login' || this.router.url === '/languages') {
  ...
  logout() {
    ...
    localStorage.setItem('cover', null);
    localStorage.setItem('email', null);
    localStorage.setItem('name', null);
    localStorage.setItem('suid', null);
    localStorage.setItem('city', null);
    localStorage.setItem('getByGroup', null);

    this.oneSignal.sendTags({ user_id: null });
