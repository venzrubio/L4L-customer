...
export class OrderDetailsPage implements OnInit {
  ...
  getOrder() {
    ...
    this.api.post('orders/getById', param).subscribe((data: any) => {
      ...
      this.getOrderDetails = data.data;
      ...
      if (data && data.status === 200 && data.data.length > 0) {
        ...
        this.newstatus = this.status[0].status
        ...
        this.payMethod = info.paid_method === 'Cash on Delivery' ? 'Cash on Delivery' : 'Cash on Pickup';
  ...
  confirmorderstat() {
    ...
    const newOrderNotes = {
      status: 1,
      value: this.util.getString('Order ') + this.util.getString('confirm by customer'),
      time: moment().format('lll'),
    };
    this.orderDetail.push(newOrderNotes);

    this.status.forEach(element => {
      if (element.status === 'delivered by driver') {
    ...
    this.api.post('orders/editList', param).subscribe((data: any) => {
      ...
      if (data && data.status === 200) {
        this.api.createOrderReceivedNotification(this.getOrderDetails);
  ...
  sendNotification(value) {
    ...
    console.log(userData.fcm_token)
    ...
    if (this.userInfo && this.userInfo.fcm_token) {
      this.api.sendNotification(this.util.getString('Your order #') + this.id + ' ' + value, this.util.getString('Order')
        + ' ' + value, userData.fcm_token, 'order')
        .subscribe((data: any) => {
  ...
  getOrderStatus(id) {
    ...
    if (item && item.length && item[0].status == 'created') {
