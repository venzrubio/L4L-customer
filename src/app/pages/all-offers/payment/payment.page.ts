...
export class PaymentPage implements OnInit {
  ...
  //  this.testing = this.cart.getOrderTax();


  //  console.log('WOWOWOWOWO >>>>>', this.testing);
        ...
        if (info && info.length > 0) {
          ...
          if (this.haveStripe) {
            ...
            if (stripe) {
              ...
              this.util.stripeCode = creds && creds.code ? creds.code : 'PHP';
            }
...
var store_id = this.cart.cart[0].store_id;
  ...
  async getCheckoutData(){
    ...
    for (let index = 0; index < storeId.length; index++) {
     ...
     findItemTotal.forEach(element => {
      ...
      var grandTotal = (total - parseFloat(this.cart.discount)) + this.cart.orderTax + parseFloat(this.cart.deliveryPrice);
  ...
  async createOrder(paymentmethod) {
    ...
    // console.log(this.cart.deliveryAt === 'home' ? JSON.stringify(this.cart.deliveryAddress) : '');
