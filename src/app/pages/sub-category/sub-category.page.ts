...
export class SubCategoryPage implements OnInit {
  ...
  getSubProducts(limit, event) {
    ...
    this.api.post('products/getByCSID', city).subscribe((cates: any) => {
      ...
      // cates.data.sort((a, b) => parseFloat(a.km) - parseFloat(b.km));

      console.log(cates);

      var uniqueNames = [];
      var uniqueObj = [];
      for(var i = 0; i< cates.data.length; i++){    
          if(uniqueNames.indexOf(cates.data[i].name) === -1){
              uniqueObj.push(cates.data[i])
              uniqueNames.push(cates.data[i].name);        
          }        
      }
      console.log(uniqueObj)
      cates.data = uniqueObj;
  ...
  add(product, index) {
    ...
    if (this.products[index].quantiy > 0) {
      ...
      this.cart.addQuantity(this.products[index].quantiy, product.id,product.store_id);
    }
  }
