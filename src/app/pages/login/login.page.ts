...
export class LoginPage implements OnInit {
  ...
  login() {
    ...
    this.oneSignal.getIds().then((res)=>{
      ...
      this.oneSignalGetID = res.userId;
    ...
    this.api.post('users/login', param).subscribe((data: any) => {
      ...
      const player = {
        id: data.data.id,
        player_id: this.oneSignalGetID

    }
    localStorage.setItem('player_id',this.oneSignalGetID);
    this.api.post('users/updateCustomerPlayerID', player).subscribe((data: any) => {
        this.loggedIn = false;
        console.log('*******************', data);
      ...
      if (data && data.status === 200) {
        if (data && data.data && data.data.type === 'user') {
          if (data.data.status === '1') {
            ...
            this.api.post('chats/getByGroup', paramg).subscribe((data: any) => { 
              localStorage.setItem('getByGroup',JSON.stringify(data));
            })

            this.oneSignal.sendTags({ user_id: data.data.id });
